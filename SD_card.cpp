...
void SD_card::SD_Setup() {
    ...
    if(dataFile) {
        ...
        dataFile.print(F("Latitude, Longitude, gpsAltitude, Course, Speed, Satellites, SeaLevelPressure = , Full_Latitude, Full_Longitude, Date, Time, Day,")); 
		
         //Status Liftoff(#21), Burnout(#22), Falling(#23), ValidBMP(#24), ValidSD(#25)
        dataFile.print(F("Liftoff, Burnout, Falling, ValidBMP, ValidSD"));
    ...
    if ((int32_t) (millis() - sd_timer) >= 0) {
        ...
        if(dataFile){
            ...
            */
            ...
            //Status Data Group
            //Print #21
            dataFile.print(status.IsLiftoff());
            dataFile.print(",");
            //Print #22
            dataFile.print(status.IsBurnout());
            dataFile.print(",");
            //Print #23
